...
const { Room, ROOM_TYPE, ROOM_STATUS } = require("./room")
...
io.on("connection", (socket) => {
  ...
  socket.on("request_game", ({ mode, difficulty }) => {
    ...
    if (mode === ROOM_TYPE.PVP) {
      ...
      AI.scheduleAIMove(room, io, 4000)
    }
  })
  ...
  socket.on("join_room", ({ roomId }) => {
    ...
    if (room) {
      ...
      // Add player
      // Determine token: if X taken, use O.
      // Note: Room.addPlayer doesn't assign token, Player constructor does.
      // We need to see what's taken.
      ...
      socket.join(roomId)
      io.to(roomId).emit("game_start", room.getGameState()) // Or update
    ...
    if (success) {
      ...
      if (room.status === ROOM_STATUS.PLAYING) {
        AI.scheduleAIMove(
          room,
          io,
          room.mode === ROOM_TYPE.PVAI
            ? 2000
            : room.getDifficulty() == "hard"
            ? 4000
            : 2000
        )
      }
